...
function deriveProviderDraftsFromDocument(
  ...
  for (const providerId of KNOWN_PROVIDER_ORDER) {
    ...
    if (
      ...
      fallbackDefaultModel.startsWith(fallbackProviderPrefix)
    ) {
      normalizedModelIds.push(
        fallbackDefaultModel.slice(fallbackProviderPrefix.length).trim(),
      );
    }
    drafts[providerId] = {
      enabled:
...
export function AuthPanel({ view = "full" }: AuthPanelProps) {
  ...
  async function persistRuntimeProviderSettings(
    ...
    try {
      ...
      for (const [providerId, providerPayload] of Object.entries(
        currentProviders,
      )) {
      ...
      for (const providerId of enabledProviders) {
        ...
        const normalizedApiKey = firstNonEmptyString(
          ...
          providerId === "holaboss"
            ...
            : "",
          providerId === "holaboss"
            ? (currentHolabossIntegration.auth_token as string | undefined)
            : "",
      ...
      if (
        source === "autosave" &&
        latestProviderDraftRevisionRef.current === draftRevision
      ) {
      ...
      >
        <div className="flex items-start justify-between gap-3 px-4 py-4">
          <div className="flex min-w-0 flex-1 items-start gap-3">
            <span className="grid h-10 w-10 shrink-0 place-items-center rounded-[14px] border border-border/35 bg-background text-foreground/86">
