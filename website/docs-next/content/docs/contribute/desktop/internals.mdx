...
description: Implementation details of Holaboss Desktop.
---
...
If you change skill discoverability or prompt shaping, inspect both the desktop serialization path and the runtime quoted-skill preparation path in `runtime/api-server/src/workspace-skills.ts` plus the host-side rendering path in `runtime/harness-host/src/pi.ts`.
