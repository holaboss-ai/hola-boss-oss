...
description: Bootstrap Holaboss Desktop from the holaOS OSS repo — one-line installer or the manual six-step flow.
---

This page is for contributors who want to run Holaboss Desktop from the `holaOS` source tree, typically because they plan to change runtime, harness, or desktop code. If you just want to use the product, download the signed installer from **[/desktop](/desktop)** instead.
