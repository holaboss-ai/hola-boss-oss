...
4. `runtime/harnesses/src/pi.ts`: builds the reduced host request that crosses from runtime code into the harness host, including the raw `thinking_value` requested for this run, runtime-prepared quoted skill payloads, and runtime-resolved workspace skill manifests.
5. `runtime/harness-host/src/index.ts`: dispatches the registered host plugin by command.
6. `runtime/harness-host/src/pi.ts`: creates the Pi session, loads file-backed skills for PI resource compatibility, injects runtime and browser tools, materializes the resolved MCP tool surface, applies local skill widening, enforces workspace boundaries, renders any runtime-prepared quoted skill blocks, maps `thinking_value` onto Pi-native reasoning settings, and maps native events back out.
...
- `runtime/api-server/src/ts-runner.ts`: per-run bootstrap. This is where the runtime applies the default tool set, adds extra tools such as `web_search` and `write_report`, resolves workspace skills into runtime-owned manifests, prepares browser/runtime/MCP state, loads operator-surface context, and builds the agent runtime config request.
- `runtime/api-server/src/agent-runtime-config.ts`: prompt and capability projection. This is where prompt layers, session-memory-backed resume context, recalled memory, operator-surface context, response-delivery policy, capability manifests, and tool visibility are composed before the harness runs. For MCP, this layer distinguishes pre-enumerated tool refs from connected server ids that still require host-side discovery.
...
- `runtime/harness-host/src/pi.ts`: the current host implementation. This is where the host consumes runtime-resolved workspace skill manifests, applies local skill widening, enforces workspace-boundary policy, injects browser and runtime tools, materializes per-server allowlisted MCP subsets or full discovered tool catalogs when a configured server has no explicit refs, and normalizes provider-native `thinking_value` choices onto Pi reasoning levels or budgets.
- `runtime/harness-host/src/pi-browser-tools.ts`: desktop browser bridge used by the current host.
- `runtime/harness-host/src/pi-runtime-tools.ts`: runtime-managed tool bridge for todo, scratchpad, `skill`, `web_search`, onboarding, cronjobs, image generation, and `write_report`. This is also where the host attaches workspace/session/input/model headers to runtime-tool calls.
- `runtime/harnesses/src/desktop-browser-tools.ts`, `runtime/harnesses/src/runtime-agent-tools.ts`, and `runtime/harnesses/src/native-web-search-tools.ts`: canonical ids and descriptions for the projected browser, runtime, and shared web-search surfaces.
...
The current prompt path supports leading slash skill references from the composer, but the expansion semantics now live in the runtime path rather than the PI host.
...
`prepareInstructionWithQuotedWorkspaceSkills(...)` in `runtime/api-server/src/workspace-skills.ts` resolves those ids against the runtime-resolved workspace skill set, renders canonical `<skill ...>` blocks, and strips the leading slash lines from the instruction body before the harness request is built.
...
The PI adapter forwards that prepared payload as:
...
`buildPiPromptPayload()` no longer resolves slash skills from raw instruction text. It only renders the already-prepared `quoted_skill_blocks` and the missing-skill warning section. Keep that behavior stable so composer-selected skills remain inspectable in runtime traces and harness prompts without making the feature semantics PI-specific.

## Runtime-backed `skill` and `web_search`

The current shipped path no longer treats `skill` or `web_search` as PI-owned executor behavior.

- `runtime/api-server/src/runtime-agent-tools.ts` owns the shared runtime capability endpoints
- `runtime/api-server/src/workspace-skills.ts` owns canonical skill resolution and skill-block rendering
- `runtime/api-server/src/native-web-search.ts` owns the current Exa-backed public-web search execution
- `runtime/harness-host/src/pi-runtime-tools.ts` only packages and proxies those tool calls for PI
...
That split is deliberate. Skill meaning is runtime-owned; local enforcement over PI-managed tools remains host-local.
