...
This page explains why the app layer exists and how it fits into the system. For manifest fields, lifecycle hooks, MCP registration, and output-publishing behavior, continue into the [Apps](/docs/build/apps) section.
...
<Cards>
  <Card title="App Anatomy" href="/docs/build/apps/app-anatomy" description="See how a workspace app is structured in practice across UI, services, manifests, integrations, and outputs." />
  <Card title="Build Your First App" href="/docs/build/apps/first-app" description="Move from the system model into the concrete path for creating a workspace app." />
  <Card title="MCP Tools" href="/docs/build/apps/mcp-tools" description="See how mcp.tools becomes workspace MCP registry state and what the runtime expects from a healthy MCP app." />
  <Card title="app.runtime.yaml" href="/docs/build/apps/app-runtime-yaml" description="See the manifest that declares lifecycle, MCP, and integration requirements for an app." />
  <Card title="Publishing Outputs" href="/docs/build/apps/publishing-outputs" description="See how apps publish durable workspace outputs and turn-scoped artifacts back into the environment." />
</Cards>
