...
In the shipped `pi` path today, tools such as `todoread`, `todowrite`, `skill`, `web_search`, scratchpad tools, onboarding tools, cronjob tools, image generation, and `write_report` all execute through the shared runtime capability API. The PI host only packages and proxies them.
