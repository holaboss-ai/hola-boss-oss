...
- use the `Defaults` panel to choose a fixed `Subagent model` for hidden delegated and scheduled subagent runs
...
- `runtime.subagents.model`
  - fixed model token used for hidden subagent execution
