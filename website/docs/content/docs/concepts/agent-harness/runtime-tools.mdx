...
| `browser_act` | Perform a general browser action such as click, hover, focus, fill, type, press, select, or scroll into view using a ref or locator. |
| `browser_wait` | Wait for page load, URL changes, visible/hidden elements, text, or DOM changes before continuing a browser workflow. |
