...
export interface PiWebSearchToolOptions {
...
function parseLivecrawl(toolParams: unknown): ExaWebSearchLivecrawlMode {
  const value = isRecord(toolParams) && typeof toolParams.livecrawl === "string" ? toolParams.livecrawl.trim().toLowerCase() : "";
...
export function createPiWebSearchToolDefinition(
  ...
  return {
    name: definition.id,
    label: "Web Search",
    description: webSearchDescription(definition.description),
    promptSnippet: `${definition.id}: ${webSearchDescription(definition.description)}`,
...
export async function resolvePiWebSearchToolDefinitions(
  ...
  return NATIVE_WEB_SEARCH_TOOL_DEFINITIONS.map((definition) =>
    createPiWebSearchToolDefinition(definition.id, options)
  );
}
